...
def measure_time(text, pattern, algorithm):
    print(f"Measuring time for {algorithm} with pattern '{pattern}' in text '{text[:30]}...'")
...
def main():
    ...
    existing_substring = "example"  # Змінити на реальний підрядок, що існує в тексті
    ...
    for text, text_name in [(text1, "article1"), (text2, "article2")]:
        for substring, substring_type in [(existing_substring, "existing"), (non_existing_substring, "non_existing")]:
    ...
    with open('search_algorithm_comparison.md', 'w', encoding='utf-8') as file:
        ...
        for result in results:
            file.write(f"| {result[0]} | {result[1]} | {result[2]} | {result[3]:.6f} |\n")
        ...
        for text_name in ["article1", "article2"]:
            for substring_type in ["existing", "non_existing"]:
                subset = [r for r in results if r[0] == text_name and r[1] == substring_type]
                fastest = min(subset, key=lambda x: x[3])
                file.write(f"- For {text_name} with a {substring_type} substring, the fastest algorithm is {fastest[2]} with a time of {fastest[3]:.6f} seconds.\n")
